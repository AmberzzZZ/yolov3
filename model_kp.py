...
def yolo_body(input_shape, n_classes):
    ...
    U0, P0 = fpn_node(C0, 512, 256, 1+n_classes*(2+1))
    U0 = UpSampling2D(size=2)(U0)
    C1 = concatenate([U0, C1], axis=-1)
    U1, P1 = fpn_node(C1, 256, 128, 1+n_classes*(2+1))
    U1 = UpSampling2D(size=2)(U1)
    C2 = concatenate([U1, C2], axis=-1)
    _, P2 = fpn_node(C2, 128, 0, 1+n_classes*(2+1))
...
if __name__ == '__main__':
    ...
    model = kp_model(input_shape=(640,128,3), n_classes=24, load_pretrained=False, weights_path='yolo.h5')
    # model.summary()
    print(model.output)
